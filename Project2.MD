...
Random forest regression is another non-parametric model that differs from locally weighted regression because it performs predictions through ensembling.  The main idea of random forest regression is that the predictions are made by running a collection of several decision trees at once and then averaging the results of their decisions together.  A decision tree consists of a a collection of nodes, with the top single node being the "root."  Each parent node then splits the data based on the criteria that the split minimizes the residual sum of squares, consisting of the sum of the quantity of differences between the y-values in the left child node and the mean y-value in that child node squared and the sum of the quantity of differences between the y-values in the right child node and the mean y-value in that child node squared.  If the datapoints fall below the value that splits the parent node, they will be placed in the left child node; if the datapoints fall above the value that splits the parent node, they will be placed in the right child node.
